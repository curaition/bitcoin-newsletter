...
class CitationValidator:
    ...
    def validate_citations(self, content: str) -> dict[str, Any]:
        ...
        institutional_pattern = (
            r"(institutional|market|regulatory).*?signal.*?\((\d+\.\d+)\)"
        )
        ...
        institutional_signals = re.findall(
            institutional_pattern, content, re.IGNORECASE
        )
    ...
    async def _check_url_accessibility(
        ...
        try:
            async with session.head(url, allow_redirects=True) as response:
                ...
                logger.debug(
                    f"URL {url} accessibility: {is_accessible} (status: {response.status})"
                )
    ...
    def generate_quality_report(
        self, content: str, content_sections: dict[str, str] = None
    ) -> dict[str, Any]:
        """
        ...
        quality_factors = {
            ...
            "bitcoin_terminology_used": sum(
                citation_metrics["bitcoin_terminology"].values()
            )
            >= 5,
            "citation_density_good": citation_metrics["citation_density"]
            >= 5.0,  # 5 per 1000 words
        }
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                citation_metrics, quality_factors
            ),
        }

    def _generate_recommendations(
        self, citation_metrics: dict[str, Any], quality_factors: dict[str, bool]
    ) -> list[str]:
        ...
        if not quality_factors["citations_adequate"]:
            recommendations.append(
                f"Add more citations (current: {citation_metrics['total_citations']}, minimum: 8)"
            )
        ...
        if not quality_factors["bitcoin_terminology_used"]:
            recommendations.append(
                "Use more Bitcoin-specific terminology for better context"
            )

        if not quality_factors["citation_density_good"]:
            recommendations.append(
                f"Increase citation density (current: {citation_metrics['citation_density']:.1f} per 1000 words)"
            )
